...
def generar_mediciones_por_material(cantidad_por_material=20, minimo=0, maximo=50):
    return {
...
def clasificar_promedio_material(promedio):
    if promedio < 8:
        return "Bajo (hay que mejorar cultura/flujo)"
    if promedio <= 15:
        return "Estable"
    return "Alto (excelente… o están reciclando hasta la paciencia)"
...
def mostrar_reporte_promedio(cantidad_por_material=20, minimo=0, maximo=50):
